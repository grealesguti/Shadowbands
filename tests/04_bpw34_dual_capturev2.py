...
ONE BPW34 photodiode, split into TWO simultaneous gains by an MCP6002 dual
op-amp (see shadowband_iec_33v_kit.pdf that ships with the detector docs). The
single diode's cathode drives BOTH inverting summing nodes at once, so the two
channels see the same light through two transimpedances read on two ADC pins:
  CH_LOW  ← amp A, LOW gain  (Rf_low  = 100 kΩ, bright / normal light — won't saturate)
  CH_HIGH ← amp B, HIGH gain (Rf_high = 1 MΩ,  dim / near-totality — shadow-band contrast)
...
Wiring (3.3 V dual-gain build — see shadowband_iec_33v_kit.pdf):
  Pi 3V3   (1)  → MCP3208 VDD/VREF, MCP6002 V+           (3.3 V analog rail)
  Pi GND   (6)  → MCP3208 AGND/DGND, MCP6002 V-, BPW34 anode
  Pi SCLK  (23) → MCP3208 CLK  (13)
  Pi MOSI  (19) → MCP3208 DIN  (11)
  Pi MISO  (21) ← MCP3208 DOUT (12)   [DIRECT — DOUT swings 0–3.3 V, no level shift]
  Pi CE0   (24) → MCP3208 /CS  (10)
  BPW34 cathode → the SHARED summing node → both MCP6002 inverting inputs:
        −A (pin2) low-gain amp  and  −B (pin6) high-gain amp
  amp A out (pin1) → MCP3208 CH0 (CH_LOW)   via 10 kΩ + 100 nF anti-alias
  amp B out (pin7) → MCP3208 CH1 (CH_HIGH)  via 10 kΩ + 100 nF anti-alias
  Feedback: amp A 100 kΩ ∥ 10 nF ;  amp B 1 MΩ ∥ 680 pF
  Shared Vref (220k/10k ≈ 0.143 V divider) → both + inputs (pin3, pin5)
...
Usage:
  ...
  python3 tests/04_bpw34_dual_capture.py \
      --seconds 60 --sample-rate 500 --rf-low 100e3 --rf-high 1e6 \
...
class DualBPW34:
    ...
    def read_frame(self):
        ...
        # Prefer the HIGH-gain estimate (lower noise per nA). Fall back to LOW when
        # HIGH is unusable: either saturated near the top rail (bright) OR pinned
        # at the floor (its small signal has fallen into the ADC's last few LSBs).
        hi_usable = (FLOOR_COUNTS < hi < SAT_COUNTS)
        best = ipd_hi if hi_usable else ipd_lo
        return {
            "LOW": lo,
            "HIGH": hi,
...
def probe_rate(dev, seconds=3.0):
    ...
    if sps < 500:
        ...
        print("        processes, or accept it — shadow bands (typ. 1–15 Hz, up to")
        print("        ~30 Hz) are still well oversampled even at ~100 sps.")
    return sps
...
def main():
    ...
    ap.add_argument("--rf-high", type=float, default=1e6,
                    help="Rf fitted on the HIGH-gain amp, ohms (default 1e6 = build value; recorded; LOCK before totality)")
    ...
    try:
        ...
        meta = {
            "sensor": "single BPW34, dual-gain via MCP6002 + MCP3208 (3.3V build)",
            "ch_low": args.ch_low, "ch_high": args.ch_high,
            "rf_low_ohm": f"{args.rf_low:.6g}", "rf_high_ohm": f"{args.rf_high:.6g}",
            "cf_low_f": "10e-9", "cf_high_f": "680e-12",   # feedback caps (build values)
            "tia_fc_low_hz": "159", "tia_fc_high_hz": "234",
            "antialias": "10k+100nF per ch (fc~159 Hz)",
            "vref_bias_v": args.vref_bias, "adc_vref_v": args.vref,
            "vref_divider": "220k/10k",
            "spi_hz": args.hz, "target_sps_per_ch": args.sample_rate,
            "sat_counts": SAT_COUNTS, "floor_counts": FLOOR_COUNTS,
        }
