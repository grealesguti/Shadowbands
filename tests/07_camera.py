...
def show_in_terminal(path, max_width=70):
    """
    Render a captured image directly in the SSH terminal using chafa,
    so framing/exposure can be checked without copying files anywhere.
    Silently skips if chafa is not installed or stdout is not a TTY.
    """
    if not sys.stdout.isatty():
        return
    try:
        subprocess.run(["chafa", "--size", f"{max_width}x24", str(path)],
                       timeout=15)
    except FileNotFoundError:
        print("  (install chafa for in-terminal preview: "
...
def main():
    ...
    parser.add_argument("--no-chafa",  action="store_true",
                        help="skip in-terminal image previews")
    ...
    if ok:
        ...
        if not args.no_chafa:
            print()
            show_in_terminal(img_path)
    else:
    ...
    if ok:
        ...
        results.append(chk("Sky snapshot", True,
                            f"{size_kb:.0f} KB — preview below, check framing"))
        if not args.no_chafa:
            print()
            show_in_terminal(sky_path)
    else:
