...
─────────────────────────────────
Test 3c — FOUR OPT101 on MCP3208 (CH0–CH3), staged bring-up + array debug.
Sibling of 03b (single channel); same utils.hw / config.yaml / style.
Works on Pi Zero W and Pi 3B+.

Circuit under test (per channel n = 0..3):
  ...
  Sensor map:  A→CH0  B→CH1  C→CH2  D→CH3   (override with --chans)
...
Usage:
  ...
  python3 tests/03c_opt101_array4_debug.py --chans 0 1 2 3
